...
def calculate_all_attributes(Data: object, sps: object, flag: object) -> object:
    """
...
#    for i in range(ntr):
    ...
    npts = len(cor_smooth)
    ilag_0 = int(np.argmax(cor_smooth)+1)
    ilag_third = int(ilag_0 + npts/6)
...
#    ntr=1
    ...
    #FFI = np.array([ 0.1, 1, 6, 10, 20]) 
    #FFI = np.array([0.1, 1, 6, 10, 15])
    FFI = np.array([1, 3, 5, 6, 8])
    # higher bounds of the different tested freq. bands
    #FFE = np.array([ 1, 6, 10, 30, NyF-0.01]) 
    #FFE = np.array([ 1, 6, 10, 15, NyF-0.01]) 
    FFE = np.array([ 3, 6, 7, 9, 10])
#    ntr = len(st)
    nf = len(FFI)
...
#    for i in range(ntr):
    ...
    npts = 2560
    n = nextpow2(2*npts-1)
    Freq1 = np.linspace(0, 1, int(n/2)) * NyF # original: Freq1 = np.linspace(0, 1, n/2) * NyF
    ...
    E4FFT = np.trapz(FFTsmooth_norm[int(3*npts/4-1):npts])
...
def get_pseudo_spectral_stuff(Data, sps):
    ...
    for v in range(0,np.size(smooth_spec,1)):
        CentroIndex=int(np.around(centeroidnpX(smooth_spec[0:800,int(v)])))
        CentoiX[v]=(Freq[int(CentroIndex)])
        CentoiX1[v]=(Freq[int(np.around(centeroidnpX(smooth_spec[0:int(CentroIndex),int(v)])))])
        CentoiX3[v]=(Freq[int(np.around(centeroidnpX(smooth_spec[int(CentroIndex):800,int(v)])+CentroIndex))])
