...
def comp_features(i):
    ...
    t0 = time.time()
    try:
        st = obspy.read(f'/data/wsd03/data_manuela/Illgraben/miniseed/{year}/*/ILL05/EHZ.D/*{str(i).zfill(3)}')
        ...
        while j < len(st1[0].data) - stt*40:
            ...
            with open(f'/data/wsd03/data_manuela/Illgraben/feature_files/{year}/ILLx5678/ILLx5_{tt.julday}_{tt.hour}-{tt.minute}-{tt.second}.{tt.microsecond}.csv', "a+") as f:
                d.to_csv(f, header=False, index=False)
            j += stt*10
            t1 = time.time()
            #print(t1-t0)
    except Exception:
        print(f"no data at {i}")
        
    return
        #print((len(st[0].data) - j)/stt)
pool = multiprocessing.Pool(24)
[print(f'{i}') for i in pool.map(comp_features, range(151, 215+1))]
