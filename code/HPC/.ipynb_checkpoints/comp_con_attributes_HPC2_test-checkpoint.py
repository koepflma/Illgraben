...
def comp_features(i):
    ...
    if not os.path.exists(f'../feature_files/{year}/ILLx8/00{i}_*.csv'):    
        st = obspy.read(f'/media/manuela/Elements/illgraben/miniseed/{year}/XP/ILL?8/EHZ.D/*00{i}')
        ...
        while j < len(st1[0].data) - stt*40:
            ...
            with open(f'../feature_files/{year}/ILLx8/00{tt.julday}_{tt.hour}-{tt.minute}-{tt.second}.{tt.microsecond}.csv', "a+") as f:
                d.to_csv(f, header=False, index=False)
            j += stt*10
    elif os.path.exists(f'../feature_files/{year}/ILLx8/00{i}_*.csv'):
        pass
...
[print(f'{i}') for i in pool.map(comp_features, range(8, 10))]
